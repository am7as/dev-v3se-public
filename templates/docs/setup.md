...
cd /cephyr/users/<cid>/Alvis/__PROJECT_SLUG__
...
```bash
cat slurm-__PROJECT_SLUG__-cpu-smoke-<jobid>.out
```
...
```bash
cat slurm-__PROJECT_SLUG__-gpu-smoke-<jobid>.out
```
...
rsync -avh --progress \
    <cid>@alvis2.c3se.chalmers.se:/cephyr/users/<cid>/Alvis/__PROJECT_SLUG__/results/ \
    .\results-from-alvis\
```
