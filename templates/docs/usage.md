# Usage — `templates` (step-by-step, zero to results)

A complete walkthrough from an empty folder to a green run manifest in
all three C3SE execution modes: Docker on your laptop, Apptainer on
...
The canonical Pixi task set is fixed across every project:
...
Once the smoke is green in all three modes, you have a working C3SE
project. From here:
...
multi-GPU, etc.), see the sibling `examples/` tree.
