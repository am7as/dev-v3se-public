...
├── src/__PACKAGE_NAME__/       the Python package (rename via scripts/instantiate.*)
│   ├── __init__.py
...
- **Every environment variable the code reads has a default.** If missing,
  ...
  message. Read `src/__PACKAGE_NAME__/config.py` for the complete list.
...
| `src/__PACKAGE_NAME__/config.py`   | Any code that needs a path imports from here.        |
| `src/__PACKAGE_NAME__/devices.py`  | Gathers CPU/GPU info without requiring torch.        |
| `src/__PACKAGE_NAME__/manifest.py` | Builds + persists the run manifest.                  |
