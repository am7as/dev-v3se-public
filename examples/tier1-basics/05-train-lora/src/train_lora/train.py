"""Minimal LoRA finetune loop.

Keep it short on purpose: loading model, wrapping with LoRA, training on
a tiny dataset, saving the adapter.
"""
...
def _load_dataset() -> Dataset:
    ...
    if not ds_id:
        return Dataset.from_list(_DEFAULT_SAMPLES)
...
def _tokenize_fn(tokenizer, max_len: int = 128):
    def fn(examples):
...
def run(out_dir: Path | None = None) -> dict[str, Any]:
    ...
    model = AutoModelForCausalLM.from_pretrained(source, token=token)
    lora_cfg = LoraConfig(
    ...
    ds = _load_dataset()
    ds_tok = ds.map(_tokenize_fn(tokenizer), batched=True, remove_columns=ds.column_names)
    ...
    # Save only the LoRA adapter (small — a few MB), not the full model.
